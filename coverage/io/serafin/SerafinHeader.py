...
class SerafinHeader:
    ...
    - ipobo <numpy.1D-array>: array with 0 values for inner nodes and 1-indexed node number for boundary nodes
        [shape = nb_nodes]
    """
    def __init__(self, title='', format_type='SERAFIN ', lang="fr", endian='>'):
        """
        ...
        if lang not in ('fr', 'en'):
            raise ValueError('Language (for Serafin variables) %s is not implemented' % lang)
        self.language = lang
    ...
    def _set_file_format_and_precision(self, file_format):
        ...
        if file_format in ('SERAFIND', '       D'):
            ...
            if file_format not in ('SERAFIN ', 'SERAFINS', 'SERAPHIN'):
                logging.warning('Format "%s" is unknown and is forced to "SERAFIN "' % file_format)
    ...
    def _compute_mesh_coordinates(self):
        ...
        if True: #settings.ENABLE_MESH_ORIGIN:
    ...
    def from_file(self, file, file_size):
        ...
        logging.debug('File is determined to be %s endian' % ('big' if self.endian == '>' else 'litte'))
        ...
        try:
            ...
            logging.debug('The file type is: "%s"' % data_format)
        ...
        logging.debug('The file has %d variables' % self.nb_var)
        file.read(4)
        ...
        self._check_dim()
        logging.debug('The file is determined to be %s' % {True: '2D', False: '3D'}[self.is_2d])
        ...
        for var_name in self.var_names:
            ...
            if name not in var_table:
                ...
                logging.warning('The %s variable name "%s" is not known (lang=%s). '
        ...
        logging.debug('Finished reading the header')
        return self